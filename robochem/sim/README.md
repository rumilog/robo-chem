...
- **Perception** — each of the four cage cameras is placed at the measured
  extrinsics from `calibration_out/` and rendered for depth and segmentation.
...
stream goes*, not how a powder behaves.
...
- The spoon reads **~25 mm** off its body origin — its point cloud includes the
  bowl while the origin is mid-handle. Expected, not an error.
